...
export function ErrorTester() {
  ...
  const testErrors = [
    {
      ...
      message: "Our authentication service is currently experiencing issues. Please try again in a few minutes.",
      type: "error" as const,
    },
    {
      title: "Google Sign-In Error", 
      message: "We couldn't sign you in with Google. Please try again or use email and password.",
      type: "error" as const,
    },
    {
      title: "Email Verification Required",
      message: "Your email address needs to be verified before you can sign in. Please check your email.",
  ...
  return (
    <div className="p-6 bg-gray-50 rounded-lg">
      <h3 className="text-lg font-semibold mb-4">Error Testing (Development Only)</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {testErrors.map((error, index) => (
          <Button
            ...
            onClick={() => {
              if (error.type === "error") {
                errorModal.showError(error.message, "Try Again", undefined, error.title);
              } else if (error.type === "warning") {
                errorModal.showWarning(error.message, "OK", undefined, error.title);
              } else {
                errorModal.showInfo(error.message, "Sign In", undefined, error.title);
              }
            }}
