...
import { XCircle, FileText, Search, Download, Eye, AlertTriangle, RefreshCw } from "lucide-react";
...
const mockRejectedKYC: RejectedKYC[] = [
  {
    ...
    documentsCount: 2,
    rejectionReason: "ID document is not clear, please resubmit with higher quality image",
  ...
  },
  {
    ...
    documentsCount: 3,
    rejectionReason: "Proof of address document is expired. Please provide recent utility bill or bank statement",
    resubmissionAllowed: true,
    hasResubmitted: true
  },
  {
    ...
    reviewedBy: "Admin User",
    documentsCount: 4,
    rejectionReason: "Professional license verification failed. Invalid license number",
...
export default function RejectedKYCPage() {
  ...
  const [resubmissionFilter, setResubmissionFilter] = useState<"all" | "allowed" | "not_allowed" | "resubmitted">("all");
  ...
  const filteredRecords = rejectedKYC.filter(record => {
    ...
    const matchesResubmission = 
      ...
      (resubmissionFilter === "allowed" && record.resubmissionAllowed && !record.hasResubmitted) ||
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Rejected KYC Records</h1>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Can Resubmit</CardTitle>
            <RefreshCw className="h-4 w-4 text-yellow-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {rejectedKYC.filter(r => r.resubmissionAllowed && !r.hasResubmitted).length}
            </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Resubmitted</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Under new review</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Permanently Rejected</CardTitle>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Find rejected KYC records by user name, email, type, or resubmission status
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            ...
            <select
              ...
              onChange={(e) => setResubmissionFilter(e.target.value as "all" | "allowed" | "not_allowed" | "resubmitted")}
              className="px-3 py-2 border rounded-md"
            >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{record.documentsCount}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Submitted</p>
                        <p className="font-medium">{new Date(record.submissionDate).toLocaleDateString()}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Rejected</p>
                        <p className="font-medium">{new Date(record.rejectionDate).toLocaleDateString()}</p>
                      </div>
                      <div>
      ...
      {filteredRecords.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No rejected records found</h3>
            <p className="text-muted-foreground">
              {searchTerm || typeFilter !== "all" || resubmissionFilter !== "all"
