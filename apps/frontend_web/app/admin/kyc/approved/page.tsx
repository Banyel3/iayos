...
import { CheckCircle, FileText, Search, Download, Eye, Calendar } from "lucide-react";
...
export default function ApprovedKYCPage() {
  ...
  const filteredRecords = approvedKYC.filter(record => {
    ...
    const matchesReviewer = reviewerFilter === "all" || record.reviewedBy === reviewerFilter;
    
    return matchesSearch && matchesType && matchesReviewer;
  });

  const uniqueReviewers = Array.from(new Set(approvedKYC.map(r => r.reviewedBy)));

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Approved KYC Records</h1>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {approvedKYC.filter(r => new Date(r.approvalDate).getMonth() === new Date().getMonth()).length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round(approvedKYC.reduce((acc, r) => acc + r.processingDays, 0) / approvedKYC.length)} days
            </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Workers Approved</CardTitle>
            <CheckCircle className="h-4 w-4 text-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-6">
                  <div className="text-center">
                    ...
                    <p className="text-lg font-semibold">{record.documentsCount}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm text-muted-foreground">Processing Time</p>
                    <p className="text-lg font-semibold text-green-600">{record.processingDays} days</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm text-muted-foreground">Approved Date</p>
                    <p className="text-sm">{new Date(record.approvalDate).toLocaleDateString()}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm text-muted-foreground">Reviewed By</p>
                    <p className="text-sm font-medium">{record.reviewedBy}</p>
                  </div>
      ...
      {filteredRecords.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No approved records found</h3>
            <p className="text-muted-foreground">
              {searchTerm || typeFilter !== "all" || reviewerFilter !== "all" 
