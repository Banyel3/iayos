...
import { Calendar, MapPin, DollarSign, Clock, AlertCircle, Package, Building2, User, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
export default function InviteJobCard({ job }: InviteJobCardProps) {
  ...
  const StatusIcon = statusIcons[job.inviteStatus as keyof typeof statusIcons] || AlertCircle;
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md border-2 hover:shadow-lg transition-shadow ${
      job.inviteStatus === 'REJECTED' ? 'opacity-75' : ''
    }`}>
      {/* Header with Status */}
      <div className={`px-6 py-4 border-b ${
        job.inviteStatus === 'PENDING' ? 'bg-yellow-50 border-yellow-200' :
        job.inviteStatus === 'ACCEPTED' ? 'bg-green-50 border-green-200' :
        job.inviteStatus === 'REJECTED' ? 'bg-red-50 border-red-200' :
        'bg-blue-50 border-blue-200'
      }`}>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-2">
              <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${statusColors[job.inviteStatus as keyof typeof statusColors]}`}>
            ...
            <p className="text-sm text-gray-600">
              Sent on {new Date(job.createdAt).toLocaleDateString('en-US', { 
                month: 'long', 
                day: 'numeric', 
                year: 'numeric' 
              })}
      ...
      <div className="px-6 py-5">
        ...
        {job.inviteTarget && (
          <div className="flex items-center space-x-3 mb-4 p-4 bg-gray-50 rounded-lg">
            <div className={`h-12 w-12 rounded-full flex items-center justify-center ${
              job.inviteTarget.type === 'AGENCY' ? 'bg-blue-600' : 'bg-purple-600'
            } text-white`}>
            ...
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider">
                {job.inviteTarget.type === 'AGENCY' ? 'Agency Invited' : 'Worker Invited'}
              </p>
              <p className="font-semibold text-gray-900">{job.inviteTarget.name}</p>
            </div>
          </div>
        )}
        ...
        <div className={`mb-4 p-4 rounded-lg ${
          job.inviteStatus === 'PENDING' ? 'bg-yellow-50 border border-yellow-200' :
          job.inviteStatus === 'ACCEPTED' ? 'bg-green-50 border border-green-200' :
          job.inviteStatus === 'REJECTED' ? 'bg-red-50 border border-red-200' :
          'bg-blue-50 border border-blue-200'
        }`}>
          <p className={`text-sm font-medium flex items-center space-x-2 ${
            job.inviteStatus === 'PENDING' ? 'text-yellow-800' :
            job.inviteStatus === 'ACCEPTED' ? 'text-green-800' :
            job.inviteStatus === 'REJECTED' ? 'text-red-800' :
            'text-blue-800'
          }`}>
          ...
          {job.inviteRespondedAt && (
            <p className="text-xs text-gray-600 mt-1">
              Responded on {new Date(job.inviteRespondedAt).toLocaleDateString('en-US', { 
                month: 'long', 
                day: 'numeric', 
                year: 'numeric',
                hour: '2-digit',
        ...
        <div className={`mb-4 p-4 rounded-lg border ${
          job.inviteStatus === 'REJECTED' ? 'bg-gray-50 border-gray-200' : 'bg-green-50 border-green-200'
        }`}>
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-semibold text-gray-900 flex items-center">
              <DollarSign className={`h-5 w-5 mr-2 ${job.inviteStatus === 'REJECTED' ? 'text-gray-600' : 'text-green-600'}`} />
              Payment Details
            </h4>
            <span className={`text-2xl font-bold ${job.inviteStatus === 'REJECTED' ? 'text-gray-600' : 'text-green-600'}`}>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${job.inviteStatus === 'REJECTED' ? 'text-gray-700' : 'text-green-700'}`}>
          ...
          {job.inviteStatus === 'PENDING' && (
            <div className="mt-3 pt-3 border-t border-green-300">
              <p className="text-xs text-gray-600">
                💰 Your downpayment is held in escrow until the invitation is accepted.
              </p>
            </div>
          )}
        </div>
        ...
        {job.inviteStatus === 'ACCEPTED' && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            <button
              onClick={() => window.location.href = `/client/jobs/${job.jobID}`}
