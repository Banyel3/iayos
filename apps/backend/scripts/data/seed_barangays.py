...
Run from src/ directory: python seed_barangays.py
"""
...
def seed_zamboanga_data():
    ...
    if created:
        ...
        print(f"ℹ️  City already exists: {city.name} (ID: {city.cityID})")
    ...
    print(f"\n📋 Seeding {len(barangays)} barangays...")
    ...
    if existing_count > 0:
        print(f"  ℹ️  {existing_count} barangays already existed")
    
    print(f"\n📊 Summary:")
    print(f"   City: {city.name} (ID: {city.cityID})")
    print(f"   Total barangays: {len(barangays)}")
    print(f"   Newly created: {created_count}")
    print(f"   Already existing: {existing_count}")
    print(f"\n✅ Seeding completed successfully!")
    print(f"\n💡 Use City ID {city.cityID} in the mobile app to fetch barangays")
