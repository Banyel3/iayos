...
def get_reviewer_info(
    account: Accounts, profile_type: Optional[str] = None
    ...
    try:
        ...
        if profile_type:
            profile = Profile.objects.filter(
                accountFK=account, profileType=profile_type
            ).first()
        else:
    ...
    try:
        if review.revieweeID:
            profile = (
                ...
                or Profile.objects.filter(accountFK=review.revieweeID).first()
            )
            if profile:
                full_name = (
    ...
    try:
        ...
        try:
            ...
            if user_profile and user_profile.latitude and user_profile.longitude:
                ...
                print(
                    f"📍 [LOCATION] User location ({profile_type}): {user_lat}, {user_lon}"
                )
        ...
        queryset = queryset.select_related(
            "clientID__profileID__accountFK", "categoryID"
        ...
        for job in all_jobs:
            ...
            try:
                ...
                if profile and hasattr(profile, "workerprofile"):
                    has_applied = JobApplication.objects.filter(
                        jobID=job, workerID__profileID__accountFK=user
                    ).exists()
            except Exception:
                pass
            ...
            client_name = (
                f"{client_profile.firstName} {client_profile.lastName}"
                if client_profile
                else "Unknown Client"
            )
            ...
            job_data = {
                ...
                "client_avatar": client_profile.profileImg
                if client_profile and client_profile.profileImg
                else None,
                ...
                "total_workers_needed": team_workers_needed
                if job.is_team_job
                else None,
                "total_workers_assigned": team_workers_assigned
                if job.is_team_job
                else None,
                "team_fill_percentage": team_fill_percentage
                if job.is_team_job
                else None,
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Failed to fetch jobs: {str(e)}"}


def _compute_escrow_status(job) -> Optional[Dict[str, Any]]:
    """
    Compute the escrow health for an active job.

    For DAILY jobs (regular and team):
      - escrow_deposited = daily_escrow_total
      - escrow_consumed  = sum of processed attendance payments + early completion payouts
      - escrow_required  = remaining obligation across all active workers
      - needs_top_up     = escrow_remaining < escrow_required

    For PROJECT jobs:
      - escrow_deposited = escrowAmount if paid
      - remaining_required = remainingPayment if not yet paid
      - needs_top_up = client wallet cannot cover the remaining payment

    Returns None if the computation is not applicable (e.g. unknown payment model).
    """
    from .models import Wallet, DailyAttendance, JobWorkerAssignment

    payment_model = getattr(job, "payment_model", "PROJECT") or "PROJECT"

    if payment_model == "DAILY":
        escrow_deposited = job.daily_escrow_total or Decimal("0.00")

        # Sum all processed attendance payments for this job
        total_attendance_paid = DailyAttendance.objects.filter(
            jobID=job, payment_processed=True
        ).aggregate(total=Sum("amount_earned"))["total"] or Decimal("0.00")

        # Sum early-completion lump-sum payouts (Panelist #2)
        total_early_payouts = Decimal("0.00")
        if job.is_team_job:
            early_completed_assignments = JobWorkerAssignment.objects.filter(
                jobID=job, early_completed=True
            )
            for a in early_completed_assignments:
                total_early_payouts += a.early_completion_payout or Decimal("0.00")

        escrow_consumed = total_attendance_paid + total_early_payouts
        escrow_remaining = escrow_deposited - escrow_consumed

        # Compute remaining obligation
        duration_days = job.duration_days or 0
        escrow_required = Decimal("0.00")

        if job.is_team_job:
            # Team job: sum per-worker remaining obligations
            active_assignments = JobWorkerAssignment.objects.filter(
                jobID=job, assignment_status="ACTIVE", early_completed=False
            )
            for a in active_assignments:
                rate = a.daily_rate_at_assignment or Decimal("0.00")
                worked = a.days_worked or 0
                remaining = max(duration_days - worked, 0)
                escrow_required += rate * remaining
        else:
            # Regular (single-worker) daily job
            rate = job.daily_rate_agreed or Decimal("0.00")
            worked = job.total_days_worked or 0
            remaining = max(duration_days - worked, 0)
            escrow_required += rate * remaining

        needs_top_up = escrow_remaining < escrow_required
        top_up_amount = max(escrow_required - escrow_remaining, Decimal("0.00"))

        return {
            "payment_model": "DAILY",
            "escrow_deposited": float(escrow_deposited),
            "escrow_consumed": float(escrow_consumed),
            "escrow_remaining": float(escrow_remaining),
            "escrow_required_for_completion": float(escrow_required),
            "needs_top_up": needs_top_up,
            "top_up_amount": float(top_up_amount),
        }

    elif payment_model == "PROJECT":
        escrow_deposited = float(job.escrowAmount) if job.escrowPaid else 0.0
        remaining_required = (
            float(job.remainingPayment) if not job.remainingPaymentPaid else 0.0
        )

        # Check client wallet balance
        client_available = 0.0
        try:
            client_account = job.clientID.profileID.accountFK
            wallet = Wallet.objects.get(accountFK=client_account)
            client_available = float(wallet.availableBalance)
        except Wallet.DoesNotExist:
            pass

        needs_top_up = client_available < remaining_required
        top_up_amount = max(remaining_required - client_available, 0.0)

        return {
            "payment_model": "PROJECT",
            "escrow_deposited": escrow_deposited,
            "escrow_consumed": 0.0,  # PROJECT escrow is held until completion
            "escrow_remaining": escrow_deposited,
            "escrow_required_for_completion": remaining_required,
            "needs_top_up": needs_top_up,
...
def get_mobile_job_detail(job_id: int, user: Accounts) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"   Client Profile ID: {job.clientID.profileID.accountFK.accountID if job.clientID else 'None'}"
        )
        ...
        client_reviews = JobReview.objects.filter(
            revieweeProfileID=client_profile, status="ACTIVE"
        )
        ...
        client_data = {
            ...
            "phone": client_profile.contactNum
            if client_profile and client_profile.contactNum
            else None,
        }
        ...
        if job.materialsNeeded:
            try:
                materials_needed = (
                    json.loads(job.materialsNeeded)
                    if isinstance(job.materialsNeeded, str)
                    else job.materialsNeeded
                )
            except:
        ...
        if job.jobType == "INVITE" and job.assignedWorkerID:
            ...
            try:
                ...
                worker_reviews = JobReview.objects.filter(
                    revieweeProfileID=worker_profile, status="ACTIVE"
                )
                # Fallback for old reviews
                ...
                worker_avg_rating = worker_reviews.aggregate(Avg("rating"))[
                    "rating__avg"
                ]
                ...
                assigned_worker = {
                    ...
                    if worker_profile and worker_profile.profileImg
                    else None,
                    "rating": round(worker_rating, 2),
                    "phone": worker_profile.contactNum
                    if worker_profile and worker_profile.contactNum
                    else None,
                }
                print(
                    f"   ✓ Assigned worker: {assigned_worker['name']}, Rating: {worker_rating:.2f}"
                )
            except Exception as e:
        ...
        if job.assignedAgencyFK:
            try:
                ...
                agency_reviews = JobReview.objects.filter(
                    revieweeID=agency.accountFK, status="ACTIVE"
                )
                agency_avg_rating = agency_reviews.aggregate(Avg("rating"))[
                    "rating__avg"
                ]
                ...
                assigned_agency = {
                    ...
                    if hasattr(job, "worker_assignments")
                    else 0,
                }
                print(
                    f"   ✓ Assigned agency: {assigned_agency['name']}, Workers: {assigned_agency['workers_assigned']}"
                )
            except Exception as e:
        ...
        if job.status == "COMPLETED":
            ...
            def build_account_snapshot(account: Accounts) -> Optional[Dict[str, Any]]:
                ...
                name = (
                    ""
                    if not profile
                    else f"{profile.firstName or ''} {profile.lastName or ''}".strip()
                )
                snapshot = {
            ...
            reviews_qs = JobReview.objects.filter(
                jobID=job, status="ACTIVE"
            ).select_related("reviewerID", "revieweeID")
            for review in reviews_qs:
                # Skip reviews with missing reviewer or reviewee
                if not review.reviewerID or not review.revieweeID:
                    print(
                        f"   ⚠️ Skipping review {review.reviewID} - missing reviewer or reviewee"
                    )
                    continue
        ...
        try:
            ...
            if prediction and prediction.get("predicted_hours") is not None:
                ...
                print(
                    f"   ✓ ML Prediction: {ml_prediction['formatted_duration']} (confidence: {ml_prediction['confidence_level']:.0%})"
                )
        except Exception as e:
        ...
        try:
            ...
            if (
                user_profile_for_dist
                and user_profile_for_dist.latitude
                and user_profile_for_dist.longitude
            ):
                ...
                if job_lat and job_lon:
                    job_distance = calculate_distance(
                        user_profile_for_dist.latitude,
                        user_profile_for_dist.longitude,
                        job_lat,
                        job_lon,
                    )
        ...
        client_confirmed_work_started_at = getattr(
            job, "clientConfirmedWorkStartedAt", None
        )
        ...
        job_data = {
            ...
            if getattr(job, "scheduled_end_date", None)
            else None,
            ...
            "worker_marked_on_the_way_at": worker_marked_on_the_way_at.isoformat()
            if worker_marked_on_the_way_at
            else None,
            "worker_marked_job_started": worker_marked_job_started,
            "worker_marked_job_started_at": worker_marked_job_started_at.isoformat()
            if worker_marked_job_started_at
            else None,
            "client_confirmed_work_started": client_confirmed_work_started,
            "client_confirmed_work_started_at": client_confirmed_work_started_at.isoformat()
            if client_confirmed_work_started_at
            else None,
        ...
        if job.is_team_job:
            ...
            if "error" not in team_detail:
                job_data.update(
                    {
                        ...
                        "team_fill_percentage": team_detail.get(
                            "team_fill_percentage", 0
                        ),
                        "total_workers_needed": team_detail.get(
                            "total_workers_needed", 0
                        ),
                        "total_workers_assigned": team_detail.get(
                            "total_workers_assigned", 0
                        ),
                        "budget_allocation_type": team_detail.get(
                            "budget_allocation_type"
                        ),
                        "team_start_threshold": team_detail.get(
                ...
                print(
                    f"   ✅ Team job data: {len(job_data.get('skill_slots', []))} slots, {job_data.get('total_workers_assigned', 0)}/{job_data.get('total_workers_needed', 0)} workers"
                )
            else:
        ...
        # ── Escrow Status Computation (for client on active jobs) ──
        # Tells the client whether the current escrow covers what the job
        # still requires.  Works for both DAILY and PROJECT payment models.
        if (
            job.status in ("IN_PROGRESS", "ASSIGNED")
            and job.clientID
            and job.clientID.profileID.accountFK == user
        ):
            try:
                escrow_status = _compute_escrow_status(job)
                if escrow_status:
                    job_data["escrow_status"] = escrow_status
            except Exception as e:
                print(f"   ⚠️ Escrow status computation error: {e}")

        if reviews_payload:
            job_data["reviews"] = reviews_payload

        print(
            f"   ✅ Returning job data: ID={job_data['id']}, Title={job_data['title']}"
        )
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Failed to fetch job details: {str(e)}"}
...
def create_mobile_job(user: Accounts, job_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        commission_fee = budget * Decimal(
            "0.10"
        )  # 10% platform fee (consistent with web)
        ...
        if job_data.get("category_id"):
            try:
                category = Specializations.objects.get(
                    specializationID=job_data["category_id"]
                )
        ...
        job = JobPosting.objects.create(
            ...
            preferredStartDate=datetime.fromisoformat(job_data["preferred_start_date"])
            if job_data.get("preferred_start_date")
            else None,
            scheduled_end_date=datetime.strptime(
                job_data["scheduled_end_date"], "%Y-%m-%d"
            ).date()
            if job_data.get("scheduled_end_date")
            else None,
        ...
        if payment_method == "WALLET":
            ...
            try:
                ...
                print(
                    f"   Budget: ₱{budget}, Escrow: ₱{escrow_amount}, Platform Fee: ₱{platform_fee}"
                )
                print(f"   Total to reserve: ₱{total_to_charge}")
                print(
                    f"   Wallet balance: ₱{wallet.balance}, Reserved: ₱{wallet.reservedBalance}, Available: ₱{wallet.availableBalance}"
                )
                ...
                with db_transaction.atomic():
                    ...
                    print(
                        f"   ✅ Reserved ₱{total_to_charge} - New reserved balance: ₱{wallet.reservedBalance}"
                    )
                ...
                traceback.print_exc()
                job.delete()  # Rollback
                return {"success": False, "error": f"Wallet payment failed: {str(e)}"}
        else:
...
def create_mobile_invite_job(
    user: Accounts, job_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not worker_id and not agency_id:
            return {
                "success": False,
                "error": "Must provide either worker_id or agency_id",
            }
        ...
        if worker_id:
            try:
                assigned_worker = WorkerProfile.objects.get(
                    profileID__profileID=worker_id
                )
        ...
        if is_multi_employee:
            ...
            if len(skill_slots_data) == 0:
                return {
                    "success": False,
                    "error": "At least one skill slot is required for multi-employee mode",
                }

            total_workers = sum(
                slot.get("workers_needed", 1) for slot in skill_slots_data
            )
            if total_workers < 1:
                return {
                    "success": False,
                    "error": "Total workers needed must be at least 1",
                }
            if total_workers > 20:
            ...
            for slot in skill_slots_data:
                ...
                if not spec_id:
                    return {
                        "success": False,
                        "error": "Each skill slot must have a specialization_id",
                    }
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Invalid specialization_id: {spec_id}",
                    }
                ...
                if workers_needed < 1 or workers_needed > 10:
                    return {
                        "success": False,
                        "error": "workers_needed must be between 1 and 10",
                    }

            # Use first slot's specialization as job category (for compatibility)
            category = Specializations.objects.get(
                specializationID=skill_slots_data[0]["specialization_id"]
            )
            print(
                f"📋 Multi-employee mode: {len(skill_slots_data)} skill slots, {total_workers} total workers"
            )
        else:
            ...
            if assigned_worker:
                ...
                worker_skill_ids = [
                    ws.specializationID.specializationID for ws in worker_skills
                ]

                if len(worker_skill_ids) == 0:
                    return {
                        "success": False,
                        "error": "Worker has no skills registered. They must add at least one skill to receive job invites.",
                    }
                ...
                if not category_id:
                    ...
                    if len(worker_skill_ids) == 1:
                        ...
                        print(
                            f"📋 Auto-selected category '{category.specializationName}' from worker's single skill"
                        )
                    else:
                ...
                if not category_id:
                    return {
                        "success": False,
                        "error": "category_id is required for single-employee mode",
                    }
                try:
        ...
        print(
            f"💰 Budget: ₱{budget} | Escrow: ₱{escrow_amount} | Commission: ₱{commission_fee} | Downpayment: ₱{downpayment_amount}"
        )
        ...
        if preferred_start_date_str:
            try:
                preferred_start_date_obj = datetime.strptime(
                    preferred_start_date_str, "%Y-%m-%d"
                ).date()
            except ValueError:
                return {
                    "success": False,
        ...
        if scheduled_end_date_str:
            try:
                scheduled_end_date_obj = datetime.strptime(
                    scheduled_end_date_str, "%Y-%m-%d"
                ).date()
            except ValueError:
                return {
                    "success": False,
        ...
        if (
            project_duration_days <= 0
            and preferred_start_date_obj
            and scheduled_end_date_obj
        ):
            project_duration_days = max(
                1, (scheduled_end_date_obj - preferred_start_date_obj).days + 1
            )

        if project_duration_days <= 0:
            project_duration_days = _parse_expected_duration_days(
                job_data.get("expected_duration")
            )

        # Get client wallet
        wallet, created = Wallet.objects.get_or_create(
            accountFK=user, defaults={"balance": Decimal("0.00")}
        )
                ...
                if is_multi_employee:
                    ...
                    for slot_data in skill_slots_data:
                        ...
                        JobSkillSlot.objects.create(
                            ...
                            skill_level_required=slot_data.get(
                            ...
                            budget_allocated=Decimal(
                                "0"
                            ),  # No per-slot budget allocation - agency handles payroll
                        )
                    print(
                        f"✅ Created {len(skill_slots_data)} skill slots for multi-employee job"
                    )
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Invite job creation failed: {str(e)}"}
...
def delete_mobile_job(job_id: int, user: Accounts) -> Dict[str, Any]:
    ...
    try:
        ...
        if job_owner_account.accountID != user.accountID:
            print(
                f"   ❌ Permission denied - User {user.accountID} is not job owner {job_owner_account.accountID}"
            )
            return {
                "success": False,
        ...
        with db_transaction.atomic():
            ...
            transaction_count = Transaction.objects.filter(
                relatedJobPosting=job
            ).update(description=f"[JOB DELETED] {job.title}")
            ...
            try:
                ...
                budget_amount = (
                    Decimal(str(job.budget)) if job.budget else Decimal("0.00")
                )
                platform_fee = (
                    budget_amount * settings.PLATFORM_FEE_RATE
                    if budget_amount > 0
                    else Decimal("0.00")
                )
                ...
                if total_reserved > 0:
                    if job.escrowPaid:
                        ...
                        Transaction.objects.filter(
                            relatedJobPosting=job, status="PENDING"
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Failed to delete job: {str(e)}"}


def update_mobile_job(
    job_id: int, user: Accounts, job_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if (
            "budget" in job_data
            and job_data["budget"] is not None
            and has_pending_applications
        ):
            print(
                f"   ❌ Cannot change budget - Job has {pending_count} pending applications"
            )
            return {
                "success": False,
        ...
        with db_transaction.atomic():
            ...
            if "category_id" in job_data and job_data["category_id"] is not None:
                try:
                    new_category = Specializations.objects.get(
                        specializationID=job_data["category_id"]
                    )
                    if job.categoryID != new_category:
                        old_cat_name = (
                            job.categoryID.specializationName
                            if job.categoryID
                            else "None"
                        )
                        ...
                        changes.append(
                            f"Category: {old_cat_name} → {new_category.specializationName}"
                        )
            ...
            if "budget" in job_data and job_data["budget"] is not None:
                ...
                if new_budget != old_budget:
                    ...
                    print(
                        f"   💰 Budget change: ₱{old_budget} → ₱{new_budget} (diff: ₱{budget_difference})"
                    )
                    ...
                    if budget_difference > 0:
                        ...
                        print(
                            f"   ✅ Reserved additional ₱{additional_needed} - New reserved: ₱{wallet.reservedBalance}"
                        )
                        ...
                        wallet.reservedBalance = max(
                            Decimal("0.00"), wallet.reservedBalance - excess_to_release
                        )
                        wallet.save()
                        ...
                        print(
                            f"   ✅ Released ₱{excess_to_release} - New reserved: ₱{wallet.reservedBalance}"
                        )
            ...
            if (
                "expected_duration" in job_data
                and job_data["expected_duration"] is not None
            ):
                ...
                if job.expectedDuration != new_duration:
                    ...
                    changes.append(
                        f"Duration: {job.expectedDuration or 'Not set'} → {new_duration}"
                    )
            ...
            if "preferred_start_date" in job_data:
                if (
                    job_data["preferred_start_date"] is None
                    or job_data["preferred_start_date"] == ""
                ):
                    ...
                    try:
                        new_date = datetime.fromisoformat(
                            str(job_data["preferred_start_date"]).replace("Z", "+00:00")
                        )
                        if job.preferredStartDate != new_date.date():
                            old_values["preferred_start_date"] = (
                                str(job.preferredStartDate)
                                if job.preferredStartDate
                                else None
                            )
                        ...
                        return {
                            "success": False,
                            "error": "Invalid date format for preferred_start_date",
                        }
            ...
            if (
                "skill_level_required" in job_data
                and job_data["skill_level_required"] is not None
            ):
                ...
                if job.skill_level_required != new_skill:
                    ...
                    changes.append(
                        f"Skill level: {job.skill_level_required} → {new_skill}"
                    )
                    job.skill_level_required = new_skill

            if (
                "work_environment" in job_data
                and job_data["work_environment"] is not None
            ):
            ...
            if changes and has_pending_applications:
                ...
                print(
                    f"   📬 Notified {pending_count} pending applicant(s) about changes"
                )
        ...
        return {
            ...
            "applicants_notified": pending_count
            if changes and has_pending_applications
            else 0,
        }
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Failed to update job: {str(e)}"}


def search_mobile_jobs(
    query: str, user: Accounts, page: int = 1, limit: int = 20
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        queryset = (
            ...
            .select_related("clientID__profileID__accountFK", "categoryID")
            .prefetch_related("photos")
            .distinct()
            .order_by("-createdAt")
        )
        ...
        for job in jobs:
            ...
            if user_worker_profile:
                has_applied = JobApplication.objects.filter(
                    jobPostingFK=job, workerFK=user_worker_profile
                ).exists()
    ...
    try:
        ...
        if worker_id:
            ...
            worker_skill_spec_ids = list(
                workerSpecialization.objects.filter(workerID=worker).values_list(
...
def get_user_profile_mobile(user):
    ...
    try:
        ...
        if profile_type:
            profile = (
                Profile.objects.select_related("accountFK")
                .filter(accountFK=user, profileType=profile_type)
                .first()
            )
        else:
            profile = (
                Profile.objects.select_related("accountFK")
                .filter(accountFK=user)
                .first()
            )
        ...
        profile_data = {
            ...
            or "",  # Already a full public URL from upload_file
...
def upload_profile_image_mobile(user, image_file):
    ...
    try:
        ...
        updated_count = Profile.objects.filter(accountFK=user).update(
            profileImg=image_url
        )
        print(
            f"✅ Updated {updated_count} profiles with new image URL for user {user.email}"
        )
        ...
        return {"success": False, "error": f"Failed to upload profile image: {str(e)}"}
...
def get_workers_list_mobile(
    user, latitude=None, longitude=None, page=1, limit=20, category=None
):
    """
    ...
    try:
        ...
        try:
            ...
            if not user_profile:
                ...
                if any_profile:
                    print(
                        f"  ❌ User has profile but not CLIENT type: {any_profile.profileType}"
                    )
                    return {
                        "success": False,
            ...
            print(
                f"  ✓ User profile found: {user_profile.firstName} {user_profile.lastName}"
            )
        ...
        workers = (
            WorkerProfile.objects.select_related("profileID", "profileID__accountFK")
            .filter(
                profileID__accountFK__isVerified=True,
                profileID__accountFK__KYCVerified=True,
            )
            .exclude(profileID__accountFK__accountID__in=agency_owner_account_ids)
            .exclude(
                profileID__accountFK=user  # Exclude own worker profile
            )
            .filter(availability_status="AVAILABLE")
            .order_by(
                "-profileID__accountFK__verification_level",
                "-profileID__accountFK__createdAt",
            )
        )
        ...
        if category:
            ...
            print(
                f"  🏷️ Filtered by category {category}: {workers.count()} workers match"
            )
        ...
        print(
            f"  ✓ Fetching workers {offset + 1}-{offset + len(workers)} (page {page}, limit {limit})"
        )
        ...
        for idx, worker in enumerate(workers, 1):
            ...
            if latitude and longitude and profile.latitude and profile.longitude:
                ...
                lat2, lon2 = (
                    radians(float(profile.latitude)),
                    radians(float(profile.longitude)),
                )
                dlat = lat2 - lat1
                dlon = lon2 - lon1
            ...
            if not reviews.exists():
                reviews = JobReview.objects.filter(
                    revieweeID=account, reviewerType="CLIENT", status="ACTIVE"
                )
            ...
            completed_jobs = Job.objects.filter(
                assignedWorkerID=worker, status="COMPLETED"
            ).count()
            ...
            worker_data = {
                ...
                "total_earning": float(worker.totalEarningGross)
                if worker.totalEarningGross
                else 0.0,
            }
            ...
            if idx <= 3:  # Log first 3 workers
                print(
                    f"    Worker {idx}: {worker_name} - {worker.availability_status} - Rating: {average_rating:.1f} ({review_count} reviews) - Jobs: {completed_jobs}"
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Failed to fetch workers: {str(e)}"}
...
def get_worker_detail_mobile(user, worker_id):
    ...
    try:
        ...
        return {"success": False, "error": f"Failed to fetch worker details: {str(e)}"}
...
def get_worker_detail_mobile_v2(user, worker_id):
    ...
    try:
        ...
        completion_rate = (
            round((completed_jobs / total_assigned_jobs * 100), 1)
            if total_assigned_jobs > 0
            else 0.0
        )
        ...
        if avg_rating > 0:
            ...
            quality_rating = round(
                min(5.0, max(1.0, base_rating + random.uniform(-0.3, 0.3))), 1
            )
            communication_rating = round(
                min(5.0, max(1.0, base_rating + random.uniform(-0.2, 0.2))), 1
            )
            professionalism_rating = round(
                min(5.0, max(1.0, base_rating + random.uniform(-0.2, 0.3))), 1
            )
            timeliness_rating = round(
                min(5.0, max(1.0, base_rating + random.uniform(-0.4, 0.2))), 1
            )
        else:
            quality_rating = 0.0
        ...
        try:
            ...
            if user_profile and (
                ...
                lat1, lon1 = (
                    radians(float(user_profile.latitude)),
                    radians(float(user_profile.longitude)),
                )
                lat2, lon2 = (
                    radians(float(profile.latitude)),
                    radians(float(profile.longitude)),
        ...
        certifications = [
            {
                ...
                "specializationId": cert.specializationID.id
                if cert.specializationID
                else None,  # Link to workerSpecialization
            }
        ...
        worker_data = {
            ...
            "verification_level": account.verification_level,
            "joinedDate": account.createdAt.isoformat()
        ...
        print(
            f"   📊 Worker data: rating={worker_data['rating']}, jobs={worker_data['completedJobs']}, completion_rate={worker_data['completionRate']}%, verified={worker_data['verified']}"
        )
...
def get_agency_detail_mobile(user, agency_id):
    ...
    try:
            ...
            workers_list.append(
                {
                    ...
                    "lastName": " ".join(emp.name.split()[1:])
        ...
        employee_ratings = [
            float(emp.rating) for emp in agency_employees_qs if emp.rating
        ]
        employee_avg_rating = (
            sum(employee_ratings) / len(employee_ratings) if employee_ratings else 0.0
        )
        ...
        ).aggregate(avg_rating=Avg("rating"), review_count=Count("reviewID"))
        ...
        total_jobs_completed = sum(
            emp.totalJobsCompleted for emp in agency_employees_qs
        )
        ...
        agency_data = {
            ...
            "verified": account.KYCVerified or False,
            "verification_level": account.verification_level,
            "establishedDate": agency.createdAt.isoformat()
            if agency.createdAt
            else account.createdAt.isoformat(),
            "workers": workers_list,
        }

        print(
            f"   📊 Agency data: rating={agency_data['rating']}, workers={agency_data['activeWorkers']}, verified={agency_data['verified']}"
        )

        return {"success": True, "data": agency_data}
        ...
        traceback.print_exc()
        return {"success": False, "error": f"Failed to fetch agency details: {str(e)}"}
    ...
    try:
        ...
        if user_profile.profileType == "CLIENT":
            ...
            jobs_qs = (
                ...
                .prefetch_related("photos")
                .filter(clientID__profileID__accountFK=user)
            )
        ...
        for job in jobs:
            ...
            if client_profile:
                job_data["client_name"] = (
                    f"{client_profile.firstName or ''} {client_profile.lastName or ''}".strip()
                )
            ...
            if job.assignedWorkerID:
                ...
                job_data["worker_name"] = (
                    f"{worker_profile.firstName or ''} {worker_profile.lastName or ''}".strip()
                )
            ...
            print(
                f"       Job {job.jobID} data: job_type={job_data.get('job_type')}, invite_status={job_data.get('invite_status')}, assigned_worker_id={job_data.get('assigned_worker_id')}"
            )
        ...
        for idx, job in enumerate(job_list[:3], 1):  # Show first 3 jobs
            print(
                f"      Job {idx}: ID={job['job_id']}, job_type={job.get('job_type')}, invite_status={job.get('invite_status')}, assigned_worker={job.get('assigned_worker_id')}"
            )
...
def get_available_jobs_mobile(user, page=1, limit=20):
    ...
    try:
        ...
        if user_profile.profileType != "WORKER":
            return {"success": False, "error": "Only workers can view available jobs"}
        ...
        jobs_qs = (
            ...
            .exclude(jobID__in=applied_job_ids)
            .order_by("-createdAt")
        )
        ...
        return {"success": False, "error": f"Failed to fetch available jobs: {str(e)}"}
...
def submit_review_mobile(
    ...
    try:
        ...
        if review_type == "CLIENT_TO_WORKER":
            ...
            if (
                not job.clientID
                or job.clientID.profileID.accountFK.accountID != user.accountID
            ):
                return {
                    "success": False,
            ...
            if review_target == "EMPLOYEE" and employee_id:
                ...
                try:
                    assignment = JobEmployeeAssignment.objects.select_related(
                        "employee", "employee__agency"
                    ).get(assignmentID=employee_id, job=job)
                    reviewee_employee = assignment.employee
                    # Check if already reviewed this employee
                    existing = JobReview.objects.filter(
                        jobID=job, reviewerID=user, revieweeEmployeeID=reviewee_employee
                    ).exists()
                    if existing:
                        return {
                            "success": False,
                            "error": "You have already reviewed this employee",
                        }
                ...
                existing = JobReview.objects.filter(
                    jobID=job, reviewerID=user, revieweeAgencyID=reviewee_agency
                ).exists()
                if existing:
                    return {
                        "success": False,
                ...
                try:
                    ...
                    ).get(assignmentID=worker_assignment_id, jobID=job)
                    ...
                    existing = JobReview.objects.filter(
                        jobID=job, reviewerID=user, revieweeID=reviewee
                    ).exists()
                    if existing:
                        return {
                            "success": False,
            ...
            if existing:
                return {
                    "success": False,
                    "error": "You have already reviewed the client",
                }
        else:
        ...
        for rating_name, rating_value in [
            ...
            if rating_value < 1 or rating_value > 5:
                return {
                    "success": False,
                    "error": f"{rating_name} rating must be between 1 and 5",
                }
...
def get_worker_reviews_mobile(
    worker_id: int, page: int = 1, limit: int = 20
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        worker_profile = Profile.objects.filter(
            accountFK=worker_account, profileType="WORKER"
        ).first()
        ...
        reviews_qs = (
            JobReview.objects.filter(revieweeProfileID=worker_profile, status="ACTIVE")
            .select_related("reviewerID", "jobID")
            .order_by("-createdAt")
        )
        ...
        for review in reviews:
            ...
            within_backjob_window = (
                review.backjob_edit_deadline is not None
                and now < review.backjob_edit_deadline
            )
            ...
            review_list.append(
                {
                    ...
                    "updated_at": review.updatedAt.isoformat(),
                    "can_edit": can_edit,
                    "backjob_edit_deadline": review.backjob_edit_deadline.isoformat()
                    if review.backjob_edit_deadline
                    else None,
...
def get_job_reviews_mobile(job_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        reviews_qs = JobReview.objects.filter(jobID=job, status="ACTIVE").order_by(
            "-createdAt"
        )
        ...
        return {
            ...
            "data": {
                "reviews": review_list,
                "worker_review": next(
                    (r for r in review_list if r["reviewer_type"] == "CLIENT"), None
                ),
                "client_review": next(
                    (r for r in review_list if r["reviewer_type"] == "WORKER"), None
                ),
            },
        }
...
def get_my_reviews_mobile(
    user: Accounts, review_type: str = "given", page: int = 1, limit: int = 20
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        given_reviews_qs = (
            JobReview.objects.filter(reviewerID=user, status="ACTIVE")
            .select_related("revieweeID", "jobID")
            .order_by("-createdAt")
        )
        ...
        for review in given_reviews_qs[:50]:
            ...
            reviewee_id, profile_name, profile_img, review_target = get_reviewee_info(
                review
            )
        ...
        received_reviews_qs = (
            JobReview.objects.filter(revieweeID=user, status="ACTIVE")
            .select_related("reviewerID", "jobID")
            .order_by("-createdAt")
        )
        ...
        breakdown = {
            ...
            "four_star": received_reviews_qs.filter(
                rating__gte=3.5, rating__lt=4.5
            ).count(),
            "three_star": received_reviews_qs.filter(
                rating__gte=2.5, rating__lt=3.5
            ).count(),
            "two_star": received_reviews_qs.filter(
                rating__gte=1.5, rating__lt=2.5
            ).count(),
...
def get_review_stats_mobile(worker_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if worker_profile:
            ...
            reviews = JobReview.objects.filter(
                revieweeID=worker_account, reviewerType="CLIENT", status="ACTIVE"
            )
        ...
        recent_reviews = reviews.select_related("reviewerID", "jobID").order_by(
            "-createdAt"
        )[:5]
        recent_list = []
    ...
    try:
        ...
        if not within_24h and not within_backjob_window:
            if review.backjob_edit_deadline is not None:
                return {
                    "success": False,
                    "error": "Edit window has expired (7-day backjob review period ended)",
                }
            return {
                "success": False,
                "error": "Can only edit reviews within 24 hours of creation",
            }
        ...
        for field_name, value in [
            ...
            if value is not None and (value < 1 or value > 5):
                return {
                    "success": False,
                    "error": f"{field_name} must be between 1 and 5",
                }
...
def report_review_mobile(user: Accounts, review_id: int, reason: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"message": "Review reported successfully", "review_id": review_id},
        }
...
def create_user_report_mobile(
    ...
    try:
        ...
        if report_type in {"job", "review", "message"} and not related_content_id:
            return {
                "success": False,
                "error": "related_content_id is required for this report type",
            }
...
def get_my_reports_mobile(
    ...
    try:
        ...
        queryset = UserReport.objects.filter(reporterFK=user).select_related(
            "reportedUserFK"
        )
                ...
                "reports": [
                    {
                        ...
                        "reported_user_id": int(r.reportedUserFK_id)
                        if r.reportedUserFK_id
                        else None,
    ...
    try:
            ...
            for job in worker_jobs:
                ...
                if not existing_review and job.clientID:
                    ...
                    conv = Conversation.objects.filter(
                        relatedJobPosting_id=job.jobID
                    ).first()
                    ...
                    pending_jobs.append(
                        {
                            "job_id": job.jobID,
                            "job_title": job.title,
                            "completed_at": job.updatedAt.isoformat()
                            if job.updatedAt
                            else None,
        ...
        if client_profile:
            client_jobs = Job.objects.filter(
                clientID=client_profile, status="COMPLETED"
        ...
        return {
            "success": True,
            "data": {"pending_reviews": pending_jobs, "count": len(pending_jobs)},
        }
...
def get_client_detail_mobile(user, client_id):
    ...
    try:
        ...
        try:
            profile = (
                Profile.objects.filter(
                    accountFK__accountID=client_id, profileType="CLIENT"
                )
        ...
        reviews_qs = JobReview.objects.filter(
            revieweeProfileID=profile, status="ACTIVE"
        )
        ...
        if client_profile:
            ...
            total_spent = (
                jobs.filter(status="COMPLETED", budget__isnull=False).aggregate(
                    total=Sum("budget")
                )["total"]
                or 0
            )
...
def get_client_reviews_mobile(
    client_id: int, page: int = 1, limit: int = 10
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        client_profile = Profile.objects.filter(
            accountFK=client_account, profileType="CLIENT"
        ).first()
        ...
        if client_profile:
            ...
            reviews_qs = (
                JobReview.objects.filter(
                    revieweeID=client_account, reviewerType="WORKER", status="ACTIVE"
                )
                .select_related("reviewerID", "jobID")
                .order_by("-createdAt")
            )
