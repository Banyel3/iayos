...
def upload_agency_kyc(
    payload, business_permit, rep_front, rep_back, address_proof, auth_letter
):
    """
    ...
    try:
        ...
        print(
            f"📎 Files received: {', '.join(received_files) if received_files else 'NONE'}"
        )
        ...
        kyc_record, created = AgencyKYC.objects.get_or_create(
            accountFK=user, defaults={"status": "PENDING", "notes": ""}
        )
        ...
        if business_type:
            print(
                f"💼 Business type received: {business_type} (will be saved during extraction)"
            )
        ...
        if not created:
            ...
            for f in old_files:
                if f.fileURL:
                    ...
                    if file_path:
                        print(
                            f"🗑️ Deleting old file: {file_path} (from URL: {f.fileURL[:50]}...)"
                        )
            ...
            kyc_record.save()
            print(
                f"♻️ KYC status reset to PENDING (resubmission #{kyc_record.resubmissionCount}), deleted {deleted_count}/{old_files_count} old files from Supabase"
            )
        ...
        for key, file in files_map.items():
            ...
            # Upload to Supabase
            file_url = upload_agency_doc(
                file=file, file_name=unique_name, user_id=user.accountID
            )

            if not file_url:
                print(
                    f"❌ CRITICAL: File upload failed for {key}! No URL returned from Supabase."
                )
                raise ValueError(
                    f"Failed to upload {key} to storage. Please try again."
                )
            ...
            if not is_pdf:
                ...
                rep_id_type = (
                    raw_id_type if raw_id_type in valid_id_types else "FRONTID"
                )
                if rep_id_type != raw_id_type:
                    print(
                        f"   ⚠️ Unknown ID type '{raw_id_type}', defaulting to FRONTID"
                    )
                ...
                print(
                    f"   🔍 Using verification doc type: {verification_doc_type} (rep_id_type={rep_id_type})"
                )
                ...
                if key in face_required_docs:
                    ...
                    face_confidence = ai_details.get("face_detection", {}).get(
                        "confidence", 0
                    )
                ...
                if key in ocr_required_docs:
                    ocr_text = (
                        verification_result.extracted_text[:2000]
                        if verification_result.extracted_text
                        else None
                    )
                    ...
                    if ocr_text:
                        ...
                        print(
                            f"   📝 OCR extracted from {key}: {len(ocr_text)} chars, confidence={ocr_confidence:.2f}"
                        )
                        print(f"      Preview: {text_preview}...")
                    else:
                        print(
                            f"   ⚠️  NO OCR text extracted from {key} (verification_result.extracted_text was None or empty)"
                        )
                    ...
                    if (
                        key == "BUSINESS_PERMIT"
                        and keyword_check
                        and not keyword_check.get("passed")
                    ):
                        ai_status = "FAILED"
                        ...
                        failure_messages.append(ai_rejection_message)
                        print(
                            f"   ❌ OCR keyword check FAILED for {key}: missing {missing_groups}"
                        )
                ...
                confidence_display = (
                    f"{ai_confidence_score:.2f}" if ai_confidence_score else "0"
                )
                print(
                    f"   🤖 AI Result for {key}: status={ai_status}, confidence={confidence_display}"
                )
            else:
        ...
        try:
            print(
                f"🚀 [TRIGGER] Calling extraction service for AgencyKYC {kyc_record.agencyKycID}"
            )
            print(f"   Business type: {business_type}")
            print(f"   Files uploaded: {len(uploaded_files)}")
            from .kyc_extraction_service import (
                trigger_agency_kyc_extraction_after_upload,
            )
...
def get_agency_kyc_status(account_id):
    try:
        ...
        kyc_record, created = AgencyKYC.objects.get_or_create(
            accountFK=user, defaults={"status": "NOT_STARTED", "notes": ""}
        )

        # If just created or explicitly NOT_STARTED
        if kyc_record.status == "NOT_STARTED":
            return {"status": "NOT_STARTED", "message": "No KYC submission found"}
        ...
        def extract_file_path(url_or_path):
            ...
            match = re.search(
                r"/(?:object/(?:sign|public)/)?agency/(.+?)(?:\?|$)", url_or_path
            )
            if match:
            ...
            "files": [
                {
                    ...
                    "file_url": get_signed_url(
                        "agency", extract_file_path(f.fileURL), expires_in=3600
                    )
                    or f.fileURL,
                }
                for f in files
...
def create_agency_kyc_from_paths(
    account_id: int, file_map: dict, businessDesc: str | None = None
):
    """
    ...
    try:
        ...
        for key, path in file_map.items():
            ...
            created_files.append(
                {"file_type": key, "file_path": path, "file_name": file_name}
            )
    ...
    try:
        ...
        for emp in employees:
            ...
            active_jobs_count = Job.objects.filter(
                assignedEmployeeID=emp, status__in=["ASSIGNED", "IN_PROGRESS"]
            ).count()

            # Calculate completed jobs from actual job records
            completed_jobs_count = Job.objects.filter(
                assignedEmployeeID=emp, status="COMPLETED"
            ).count()
            ...
            final_rating = (
                float(avg_rating)
                if avg_rating
                else (float(emp.rating) if emp.rating else 0)
            )
            ...
            result.append(
                {
                    ...
                    "employeeOfTheMonthDate": emp.employeeOfTheMonthDate.isoformat()
                    if emp.employeeOfTheMonthDate
                    else None,
                    "employeeOfTheMonthReason": emp.employeeOfTheMonthReason,
                    "lastRatingUpdate": emp.lastRatingUpdate.isoformat()
                    if emp.lastRatingUpdate
                    else None,
...
def add_agency_employee(
    ...
    try:
        ...
        if (
            not specializations
            or not isinstance(specializations, list)
            or len(specializations) == 0
        ):
        ...
        employee = AgencyEmployee.objects.create(
            ...
            role=specializations[0]
            if specializations
            else "",  # Legacy field - first specialization
            avatar=avatar,
            rating=rating,
        )
    ...
    try:
        ...
        if email is not None:
            ...
            if candidate and all(
                c.isdigit() or c in ["+", "-", " "] for c in candidate
            ):
                ...
                if AgencyProfile.objects.filter(contactNumber=candidate).exists():
                    raise ValueError("Mobile number already in use by an agency")
                if (
                    AgencyEmployee.objects.filter(mobile=candidate)
                    .exclude(employeeID=employee_id)
                    .exists()
                ):
                    raise ValueError("Mobile number already in use by another employee")
        ...
        return {
            ...
            "rating": float(employee.rating) if employee.rating else None,
            "employeeOfTheMonth": employee.employeeOfTheMonth,
            "employeeOfTheMonthDate": str(employee.employeeOfTheMonthDate)
            if employee.employeeOfTheMonthDate
            else None,
            "employeeOfTheMonthReason": employee.employeeOfTheMonthReason,
            "lastRatingUpdate": str(employee.lastRatingUpdate)
            if employee.lastRatingUpdate
            else None,
            "totalJobsCompleted": employee.totalJobsCompleted,
            "totalEarnings": float(employee.totalEarnings),
            "isActive": employee.isActive,
...
def remove_agency_employee(account_id, employee_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            Job.objects.filter(assignedEmployeeID=employee).update(
...
def get_agency_profile(account_id):
    ...
    try:
        ...
        avg_employee_rating = employees.aggregate(avg_rating=Avg("rating"))[
            "avg_rating"
        ]
        ...
        try:
            ...
            total_revenue = Transaction.objects.filter(
                walletID=wallet, transactionType="EARNING", status="COMPLETED"
            ...
            start_30d = (now - timedelta(days=29)).replace(
                hour=0, minute=0, second=0, microsecond=0
            )
            ...
            for i in range(5, -1, -1):
                ...
                m_start = timezone.make_aware(
                    datetime.datetime(target_year, target_month, 1)
                )
                revenue_chart_data["All"].append(
                    {"name": m_start.strftime("%b"), "value": 0}
                )
            ...
            try:
                ...
                # Fill 7d
                daily_rev = (
                    completed_jobs_query.filter(finish_date__gte=start_7d)
                    .annotate(day=TruncDay("finish_date"))
                    .values("day")
                    .annotate(val=Sum("budget"))
                )
                day_map = {
                    d["day"].date(): float(d["val"] or 0) for d in daily_rev if d["day"]
                }
                ...
                # 30d
                weekly_rev = (
                    completed_jobs_query.filter(finish_date__gte=start_30d)
                    .annotate(week=TruncWeek("finish_date"))
                    .values("week")
                    .annotate(val=Sum("budget"))
                )
                week_map = {
                    w["week"].date(): float(w["val"] or 0)
                    for w in weekly_rev
                    if w["week"]
                }
                week_cursor = start_30d - timedelta(days=start_30d.weekday())
                ...
                for i in range(6):
                    ...
                    ms = timezone.make_aware(
                        datetime.datetime(target_year, target_month, 1)
                    )
                    if target_month == 12:
                        me = timezone.make_aware(
                            datetime.datetime(target_year + 1, 1, 1)
                        )
                    else:
                        me = timezone.make_aware(
                            datetime.datetime(target_year, target_month + 1, 1)
                        )

                    m_val = (
                        completed_jobs_query.filter(
                            finish_date__gte=ms, finish_date__lt=me
                ...
                rev_last_month = completed_jobs_query.filter(
                    finish_date__gte=last_month_start, finish_date__lt=last_month_end
                ).aggregate(total=Sum("budget"))["total"] or Decimal("0.00")

                if rev_last_month > 0:
                    revenue_growth = (
                        (rev_this_month - rev_last_month) / rev_last_month
                    ) * 100
            ...
            start_7d = (timezone.now() - timedelta(days=6)).replace(
                hour=0, minute=0, second=0, microsecond=0
            )
            for i in range(7):
                d = (start_7d + timedelta(days=i)).date()
        ...
        return {
            ...
            "statistics": {
                ...
                "avg_employee_rating": float(avg_employee_rating)
                if avg_employee_rating
                else None,
...
def update_agency_profile(
    account_id, business_name=None, business_desc=None, contact_number=None
):
...
def get_agency_jobs(
    account_id, status_filter=None, invite_status_filter=None, page=1, limit=20
):
    """
    ...
    try:
        ...
        # - Legacy direct-invite jobs (assignedAgencyFK=agency), OR
        # - Team jobs where this agency has at least one slot invite
        base_query = Q(assignedAgencyFK=agency) | Q(skill_slots__invited_agency=agency)
        ...
        if invite_status_filter:
            ...
            if invite_status_filter.upper() not in valid_invite_statuses:
                raise ValueError(
                    f"Invalid invite status. Must be one of: {', '.join(valid_invite_statuses)}"
                )
            ...
            if job.is_team_job:
                for slot in job.skill_slots.all():
                    slot_dict = {
                        ...
                        if slot.budget_allocated
                        else 0,
                        "skill_level_required": slot.skill_level_required,
                        "status": slot.status,
                    }
                    skill_slots_data.append(slot_dict)
                    # Collect only slots where THIS agency was invited
                    if slot.invited_agency_id == agency.agencyId:
                        agency_invited_slots_data.append(
                            {
                                "skill_slot_id": slot.skillSlotID,
                                "specialization_name": slot.specializationID.specializationName,
                                "workers_needed": slot.workers_needed,
            ...
            jobs_data.append(
                {
                    ...
                    "preferredStartDate": job.preferredStartDate.isoformat()
                    ...
                    "agency_invited_slots": agency_invited_slots_data,
...
def get_agency_job_detail(account_id, job_id):
    ...
    try:
        ...
        job_data = {
            ...
            "employeeAssignedAt": job.employeeAssignedAt.isoformat()
            if job.employeeAssignedAt
            else None,
            ...
            if hasattr(job, "completionNotes")
            else "",
...
def update_employee_rating(account_id, employee_id, rating, reason=None):
    ...
    try:
        ...
        Notification.objects.create(
            ...
            message=f"Updated {employee.name}'s rating to {rating}/5.0"
...
def set_employee_of_month(account_id, employee_id, reason):
    ...
    try:
        ...
        return {
            ...
            "employeeId": employee.employeeID,
            "employeeOfTheMonth": employee.employeeOfTheMonth,
            "employeeOfTheMonthDate": employee.employeeOfTheMonthDate.isoformat()
            if employee.employeeOfTheMonthDate
            else None,  # type: ignore[union-attr]
            "employeeOfTheMonthReason": employee.employeeOfTheMonthReason,
        }
...
def get_employee_leaderboard(account_id, sort_by="rating"):
    ...
    try:
        ...
        if sort_by not in valid_sorts:
            raise ValueError(
                f"Invalid sort_by parameter. Must be one of: {', '.join(valid_sorts)}"
            )
...
def assign_job_to_employee(
    agency_account, job_id: int, employee_id: int, assignment_notes: str = None
) -> dict:
    """
    ...
    try:
        employee = AgencyEmployee.objects.select_related("agency").get(
            employeeID=employee_id, agency=agency_account
        )
    except AgencyEmployee.DoesNotExist:
    ...
    if conflicting_job:
        raise ValueError(
            f"Employee {employee.name} is already working on '{conflicting_job}'. They must complete it before being assigned to a new job."
        )
    ...
    with transaction.atomic():
        ...
        if job.status == "ACTIVE":
            job.status = (
                "IN_PROGRESS"  # Go directly to IN_PROGRESS when employees assigned
            )
        job.save()
...
def unassign_job_from_employee(agency_account, job_id: int, reason: str = None) -> dict:
    """
    ...
    try:
        job = Job.objects.select_related("assignedEmployeeID", "assignedAgencyFK").get(
            jobID=job_id
        )
    except Job.DoesNotExist:
        raise ValueError(f"Job {job_id} not found")
    ...
    if job.status == "IN_PROGRESS":
        raise ValueError(
            "Cannot unassign employee from job that is already in progress"
        )
    ...
    print(
        f"✅ Employee {employee_name} (ID: {employee_id}) unassigned from job {job_id}"
    )
...
def validate_employee_not_working(employee, exclude_job=None, target_job=None):
    ...
    target_start = (
        getattr(target_job, "preferredStartDate", None) if target_job else None
    )
    target_end = (
        (getattr(target_job, "scheduled_end_date", None) or target_start)
        if target_start
        else None
    )
    ...
    legacy_query = Job.objects.filter(
        assignedEmployeeID=employee, status__in=["ASSIGNED", "IN_PROGRESS"]
    )
    if exclude_job:
...
def get_employee_workload(agency_account, employee_id: int) -> dict:
    ...
    legacy_assigned = Job.objects.filter(
        assignedEmployeeID=employee, status="ASSIGNED"
    ).count()

    legacy_in_progress = Job.objects.filter(
        assignedEmployeeID=employee, status="IN_PROGRESS"
    ).count()
...
def assign_employees_to_job(
    ...
    if job.inviteStatus != "ACCEPTED":
        raise ValueError(
            f"Cannot assign employees to job with invite status: {job.inviteStatus}"
        )
    ...
    employees = AgencyEmployee.objects.filter(
        employeeID__in=employee_ids, agency=agency_account
    )
    ...
    for emp in employees:
        conflicting_job = validate_employee_not_working(
            emp, exclude_job=job, target_job=job
        )
        if conflicting_job:
            raise ValueError(
                f"Employee {emp.name} is already working on '{conflicting_job}'. They must complete it before being assigned to a new job."
            )

    # Check for already assigned employees
    existing_assignments = JobEmployeeAssignment.objects.filter(
        job=job, employee_id__in=employee_ids, status__in=["ASSIGNED", "IN_PROGRESS"]
    ...
    with transaction.atomic():
        ...
        primary_employee = next(
            emp for emp in employees if emp.employeeID == primary_contact_id
        )
...
def remove_employee_from_job(
    agency_account, job_id: int, employee_id: int, reason: str = None
) -> dict:
    """
    ...
    try:
        assignment = JobEmployeeAssignment.objects.select_related("employee").get(
            job=job, employee_id=employee_id, status__in=["ASSIGNED", "IN_PROGRESS"]
        )
    ...
    with transaction.atomic():
        ...
        if was_primary:
            next_primary = (
                ...
                .exclude(assignmentID=assignment.assignmentID)
                .first()
            )
        ...
        employees.append(
            {
                ...
                "marked_complete_at": assignment.employeeMarkedCompleteAt.isoformat()
                if assignment.employeeMarkedCompleteAt
                else None,
            }
        )
...
def set_primary_contact(agency_account, job_id: int, employee_id: int) -> dict:
    """
    ...
    with transaction.atomic():
        ...
        JobEmployeeAssignment.objects.filter(job=job, isPrimaryContact=True).update(
...
def get_agency_reviews(
    account_id: int, page: int = 1, limit: int = 10, review_type: str = None
):
    """
    ...
    try:
        ...
        base_query = (
            ...
            .select_related(
                "jobID", "reviewerID", "revieweeAgencyID", "revieweeEmployeeID"
            )
            .order_by("-createdAt")
        )
        ...
        employee_reviews_count = base_query.filter(
            revieweeEmployeeID__in=employee_ids
        ).count()
        ...
        for review in reviews_page:
            ...
            reviewer_profile = (
                Profile.objects.filter(
                    accountFK=review.reviewerID, profileType="CLIENT"
                ).first()
...
def get_revenue_trends(account_id, weeks=12):
    ...
    try:
        ...
        weekly_data = (
            ...
            .annotate(week=TruncWeek("completedAt"))
            .values("week")
            .annotate(revenue=Sum("budget"), jobs=Count("jobID"))
            .order_by("week")
        )
        ...
        while current <= end_date:
            ...
            if existing:
                ...
                all_weeks.append({"date": week_str, "revenue": 0, "jobs": 0})
...
def assign_employees_to_slots(
    ...
    print(
        f"📋 assign_employees_to_slots service called: job_id={job_id}, assignments={len(assignments)}"
    )
    ...
    try:
        job = Job.objects.select_related(
            "assignedAgencyFK", "clientID__profileID__accountFK"
        ).get(jobID=job_id)
        print(
            f"   Found job: {job.title}, status={job.status}, inviteStatus={job.inviteStatus}"
        )
    except Job.DoesNotExist:
    ...
    with transaction.atomic():
        for assignment_data in assignments:
            ...
            try:
                ...
                return {
                    "success": False,
                    "error": f"Employee {employee_id} not found or not in your agency",
                }
            ...
            if conflicting_job:
                return {
                    "success": False,
                    "error": f'Employee {employee.fullName} is already working on "{conflicting_job}". They must complete it before being assigned to a new job.',
                }
            ...
            if employee_id in employee_ids_used:
                return {
                    "success": False,
                    "error": f"Employee {employee.fullName} is already assigned to another slot in this job",
                }
        ...
        try:
            primary_employee = AgencyEmployee.objects.get(
                employeeID=primary_contact_employee_id, agency=agency_account
            )
            job.assignedEmployeeID = primary_employee
        except AgencyEmployee.DoesNotExist:
        ...
        if created:
            print(
                f"✅ Created group conversation {conversation.conversationID} for multi-employee job"
            )
            ...
            print(
                f"🔧 Repaired conversation {conversation.conversationID}: added missing agency field"
            )
