...
export default function KYCUploadScreen() {
  ...
  const handleSubmit = async () => {
    ...
    // Stages in upload order: front ID → back ID → clearance doc → selfie
    const stages = [
    ...
    try {
      // ── Stage 1-4: upload each file individually ────────────────────────────
      ...
      // ── Stage 5: submit metadata + pre-uploaded URLs (no files in body) ─────
      setUploadProgress({
