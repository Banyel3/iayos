...
describe('Authentication: Login Flow', () => {
  ...
  it('should login successfully as WORKER with valid credentials', async () => {
    ...
    await waitFor(element(by.id('jobs-screen')).or(element(by.id('home-screen'))).or(element(by.id('select-role-screen'))))
      .toBeVisible()
      .withTimeout(10000);
  });
  ...
  it('should persist login session after app reload', async () => {
    ...
    await waitFor(element(by.id('jobs-screen')).or(element(by.id('home-screen'))))
      .toBeVisible()
      .withTimeout(10000);
